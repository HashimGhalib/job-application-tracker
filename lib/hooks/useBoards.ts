...
export function useBoard(initialBoard?: Board | null) {
    ...
    async function moveJob(
        ...
        setColumns((prev) => {
            ...
            let oldColumId: string | null = null;

            for (const col of newColumns) {
                const jobIndex = col.jobApplications.findIndex((j) => j._id === JobAppliationId);
                if (jobIndex !== -1 && jobIndex !== undefined) {
                    jobToMove = col.jobApplications[jobIndex];
                    oldColumId = col._id;
                    col.jobApplications = col.jobApplications.filter(
                        (j) => j._id !== JobAppliationId
                    );
                    break;
                }
            }

            if (jobToMove && oldColumId) {
                const targetColumnIndex = newColumns.findIndex(
                    (col) => col._id === newColumnId
                );
                if (targetColumnIndex !== -1 && targetColumnIndex !== undefined) {
        ...
        try {
            const result = await updateJobApplication(JobAppliationId, {
                columnId: newColumnId,
                order: newOrder,
            })
